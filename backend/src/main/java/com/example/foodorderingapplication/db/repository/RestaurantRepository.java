...
public interface RestaurantRepository extends JpaRepository<Restaurant,Long> {
    ...
    @Query("select new com.example.foodorderingapplication.dto.restaurant.RestaurantDTO(r.id,r.name,u.id) from Restaurant r join r.owner u")
    List<RestaurantDTO> findAllRestaurants();

    @Query("select new com.example.foodorderingapplication.dto.restaurant.RestaurantDTO(r.id,r.name,u.id) from Restaurant r join r.owner u where u.id= :id")
